...
#SBATCH --job-name=postProcessingMappingmm39
...
for sample in "${samples[@]}"; do
	...
	samtools view -@ 4 -b -q 10 -F 1024 "$out" -o "$mapped/${sample}_mm39_markdup_filt_nodup.bam"
	samtools index "$mapped/${sample}_mm39_markdup_filt_nodup.bam"

	# COVERAGE
	samtools coverage "$mapped/${sample}_mm39_markdup_filt_nodup.bam" > "$mapped/${sample}_mm39_final_coverage.txt"
	samtools coverage -m "$mapped/${sample}_mm39_markdup_filt_nodup.bam" > "$mapped/${sample}_mm39_final_coverage_histogram.txt"
