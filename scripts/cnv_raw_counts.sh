...
output_file="$out/summary_cnv_raw_counts.tsv"
...
for sample in "${samples[@]}"; do
    ...
    cnvnator_39=$( [ -f "$cnvnator_mm39" ] && awk '$2 ~ /^(Y|chrY):/' "$cnvnator_mm39" | wc -l)
    freec_39=$( [ -f "$freec_mm39" ] && awk '$1 == "Y"' "$freec_mm39" | wc -l)
    cnvkit_39=$( [ -f "$cnvkit_mm39" ] && awk '$1 == "Y" && $6 != 2' "$cnvkit_mm39" | wc -l)
    ...
    cnvnator_t2t=$( [ -f "$cnvnator_t2t" ] && awk '$2 ~ /^(Y|chrY):/' "$cnvnator_t2t" | wc -l)
    freec_t2t=$( [ -f "$freec_t2t" ] && awk '$1 == "Y"' "$freec_t2t" | wc -l)
    cnvkit_t2t=$( [ -f "$cnvkit_t2t" ] && awk '$1 == "Y" && $6 != 2' "$cnvkit_t2t" | wc -l)
