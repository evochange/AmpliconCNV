...
output_file="$out/summary_cnv_raw_countsX.tsv"

# DIRS CREATION
mkdir -p "$out"

# OVERWRITE OLD DATA
echo -e "Sample\tBuild\tCNVnator_Y\tCNVkit_Y\tFREEC_Y" > "$output_file"
...
for sample in "${samples[@]}"; do
    ...
    cnvnator_39=$( [ -f "$cnvnator_mm39" ] && awk '$2 ~ /^(X|chrX):/' "$cnvnator_mm39" | wc -l)
    freec_39=$( [ -f "$freec_mm39" ] && awk '$1 == "X"' "$freec_mm39" | wc -l)
    cnvkit_39=$( [ -f "$cnvkit_mm39" ] && awk '$1 == "X" && $6 != 2' "$cnvkit_mm39" | wc -l)
    ...
    cnvnator_t2t=$( [ -f "$cnvnator_t2t" ] && awk '$2 ~ /^(X|chrX):/' "$cnvnator_t2t" | wc -l)
    freec_t2t=$( [ -f "$freec_t2t" ] && awk '$1 == "X"' "$freec_t2t" | wc -l)
    cnvkit_t2t=$( [ -f "$cnvkit_t2t" ] && awk '$1 == "X" && $6 != 2' "$cnvkit_t2t" | wc -l)

    echo -e "${sample}\tt2t\t${cnvnator_t2t}\t${cnvkit_t2t}\t${freec_t2t}" >> "$output_file"
done
